        ...
        return $form->schema([
            Forms\Components\Group::make()->schema([
                Forms\Components\Section::make('Datos de la orden')->schema([
                    ...
                    Forms\Components\TextInput::make('admin_whatsapp')->label('WhatsApp admin'),
                ])->columns(2),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('created_at')->label('Creada')->dateTime('Y-m-d H:i')->sortable(),
                Tables\Columns\TextColumn::make('cliente_nombre')->label('Cliente')->searchable(),
                Tables\Columns\TextColumn::make('cliente_email')->label('Email')->toggleable(isToggledHiddenByDefault: true)->searchable(),
                Tables\Columns\TextColumn::make('plan.nombre')->label('Plan')->limit(30)->searchable(),
                Tables\Columns\TextColumn::make('aseguradora.nombre')->label('Aseguradora')->limit(20)->sortable(),
                Tables\Columns\BadgeColumn::make('moneda')->colors(['primary'])->sortable(),
                Tables\Columns\TextColumn::make('precio')
                    ->label('Precio')
                    ->formatStateUsing(fn ($state, Order $r) =>
                        $r->moneda === 'USD' ? 'US$'.number_format((float)$state, 2) : '$'.number_format((float)$state, 0)
                    )
                    ->alignEnd()
                    ->sortable(),
                ...
                // Columna Mensaje sin copyable()
                Tables\Columns\TextColumn::make('whatsapp_message')
                    ->label('Mensaje')
                    ->toggleable(isToggledHiddenByDefault: true)
                    ->limit(60)
                    ->formatStateUsing(fn ($state, Order $r) =>
                        $state ?: static::buildMessageFromOrder($r)
                    ),
            ])
            ->filters([
                ...
                SelectFilter::make('moneda')->options(['COP' => 'COP', 'USD' => 'USD']),
                Filter::make('created_at')->label('Fecha de creación')
                    ...
                    ->query(fn (Builder $q, array $d) => $q
                        ...
                        ->when($d['until'] ?? null, fn($qq,$v)=>$qq->whereDate('created_at','<=',$v))
                    ),
            ])
            ->actions([
                ...
                // Acción para copiar mensaje con JS
